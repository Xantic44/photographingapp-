...
# Inlamningsuppgift - Loopar, logik och listor
...
def del_a_guess_number():
    ...
    print("Jag har valt ett tal mellan 1 och 200.")
    print(f"Du har {max_forsok} forsok. Lycka till!\n")
    
    forsok = 1
    while forsok <= max_forsok:
        # las in gissning fran anvandaren
        svar = input("Forsok " + str(forsok) + ": ").strip()
        ...
        if gissning < hemligt_tal:
            ...
            print("Ratt! Talet var " + str(hemligt_tal) + ". Du klarade det pa " + str(forsok) + " forsok!\n")
            break
    ...
    if gissning != hemligt_tal:
        print("Tyvarr, du fick slut pa forsok. Talet var " + str(hemligt_tal) + ".\n")
    
    # skriv ut statistik
    print("--- Statistik ---")
    print("Antal forsok: " + str(len(gissningar)))
    ...
    if len(gissningar) > 0:
        ...
        print("Basta gissning: " + str(basta) + " (avstand: " + str(basta_avstand) + ")")
        print("Samsta gissning: " + str(samsta) + " (avstand: " + str(samsta_avstand) + ")")
    else:
...
def del_b_lotto():
    ...
    print()
    print("Antal ratt: " + str(len(ratt_tal)))
    
    if len(ratt_tal) > 0:
        print("Ratt gissade tal: ", end="")
...
# REFLEKTION
# -------------------------------------
# Det har varit svart men larorikt.
# Jag har lart mig om loopar, listor och felhantering.
# Min storsta utmaning var att halla koll pa alla villkor.
# Nasta gang vill jag bli battre pa att strukturera koden.